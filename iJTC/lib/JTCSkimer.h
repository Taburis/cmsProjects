...
void JTCSkimer::initTree(TTree *ct, bool ismc, bool ishi){
	...
	if(ishi){
		...
		ct->SetBranchStatus("nPFpartGT2_id1",1);
		ct->SetBranchAddress("nPFpartGT2_id1",&ncs);
...
void JTCSkimer::quickHistReg(histManager *h, TString cap, histCase &hc){
	...
	const float newbin [21] = {100, 120, 136, 152, 168, 184, 200, 216, 232, 248, 264, 280, 296, 312, 328, 344, 360,
		380, 400, 432, 500};
	int nbin = 20;
	name=cap;
...
void JTCSkimer::fillAllJetInfo(){
	...
	if(isMC) {
			...
			if(foundTaggedBJet&&foundTrueBJet)fillJetInfo(taggedTrueBCase);
	}
}
	...
	if(!isHI){ 
		for(int j=0; j<nCent; ++j){
			if(ismixing){
				...
				hc.mixing_raw[i+j*nPt]->Fill(deta, dphi, weights);
			} else {
				hc.sig[i+j*nPt]->Fill(deta, dphi, weights*trkCorr);
				hc.sig_pTweighted[i+j*nPt]->Fill(deta, dphi, weights*selectedTrack_pt*trkCorr);
				hc.sig_raw[i+j*nPt]->Fill(deta, dphi, weights);
				hc.sig_pTweighted_raw[i+j*nPt]->Fill(deta, dphi, weights*selectedTrack_pt);
			}
		}
	}
}
